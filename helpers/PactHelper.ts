...
/**
 * makeProvider — convenience factory used by payments/kyc specs.
 * Returns a PactV3 instance configured with the shared output dir.
 */
export function makeProvider(consumer: string, provider: string): PactV3 {
  return new PactV3(pactConfig(consumer, provider));
}
...
/**
 * kycSessionShape — response shape for /api/v1/kyc_sessions
 */
export function kycSessionShape(M: any) {
  return {
    id: M.string('kyc_abc'),
    session_token: M.string('kyc_abc'),
    status: M.string('pending'),
    redirect_url: M.string('https://id.thirdfort.com/verify/kyc_abc'),
    user_id: M.integer(5),
    conveyance_id: M.integer(1),
    created_at: M.iso8601DateTime('2024-01-01T00:00:00Z'),
    updated_at: M.iso8601DateTime('2024-01-01T00:00:00Z'),
